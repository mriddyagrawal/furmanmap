...
def analyze(paths):
    """Graph stats + near-miss gaps for one set of walkable paths."""
    # --- graph ------------------------------------------------------------
    ...
    return {
        ...
        "orphan_islands": len(sizes[1:]),
        "orphan_nodes": sum(len(c) for c in sizes[1:]),
        "near_miss_gaps": len(near),
    }, near, sizes[1:], coord


def main():
    all_paths = [f for f in load("paths.geojson") if f["properties"].get("walkable")]
    campus_paths = [f for f in all_paths if f["properties"].get("on_campus")]
    buildings = load("buildings.geojson")
    entrances = load("entrances.geojson")

    # Campus scope is the headline. The query bbox reaches well past campus,
    # and off-campus fringe islands made the whole-bbox figure meaningless as
    # a measure of whether a student can be routed across Furman.
    campus, near, isolated, coord = analyze(campus_paths)
    whole, _, _, _ = analyze(all_paths)

    # --- completeness -----------------------------------------------------
    on_campus_b = [f for f in buildings if f["properties"].get("on_campus")]
    named = [f for f in on_campus_b if f["properties"].get("name")]
    steps = [f for f in campus_paths if f["properties"].get("highway") == "steps"]
    names_lower = " | ".join(f["properties"]["name"].lower() for f in named)
    missing = [b for b in PHASE1_BUILDINGS if b.lower() not in names_lower]
    stats = dict(campus,
                 buildings=len(on_campus_b), buildings_named=len(named),
                 buildings_in_bbox=len(buildings),
                 entrances=len([f for f in entrances
                                if f["properties"].get("on_campus")]),
    ...
    w("_Scoped to Furman's OSM campus boundary (relation `amenity=university`,")
    w("name `Furman University`); whole-bbox figures shown for contrast._\n")
    w("## Network (on campus)\n")
    w("| Metric | Value |")
    w("|---|---|")
    ...
    w("\n_Whole query bbox for contrast: %s components, %s%% in the largest,"
      " %s orphan nodes — the extra islands are off-campus fringe._\n"
      % (whole["components"], whole["largest_component_pct"],
         whole["orphan_nodes"]))
